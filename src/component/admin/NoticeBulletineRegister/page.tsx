...
const NoticeBulletineRegister = () => {
    ...
    input.onchange = async () => {
      ...
      if (imageUrl) {
        ...
        setTimeout(() => {
          ...
          const images = editorElement.querySelectorAll(`img[src="${imageUrl}"]`);
  ...
  const handleCancel = () => {
    const hasContent = formData.title.trim() || formData.content.trim() || files.length > 0;
    
    if (hasContent && !confirm('작성 중인 내용이 있습니다. 정말 취소하시겠습니까?')) {
      return;
    }

...
export const useNavigation = (
  ...
  const handlePrevious = async () => {
    try {
      console.log(
        `[${type}] 이전글 이동 시도 - 현재 ID: ${currentId}, 쿼리키: ${config.queryKey}`
      );
      ...
      if (result.success && result.nextId) {
        // 상태 업데이트 전에 유효성 검사
        console.log(
          `[${type}] 이전글 이동: ${currentId} -> ${result.nextId}, 카테고리: ${config.targetCategory}`
        );
  ...
  const handleNext = async () => {
    try {
      console.log(
        `[${type}] 다음글 이동 시도 - 현재 ID: ${currentId}, 쿼리키: ${config.queryKey}`
      );
      ...
      if (result.success && result.nextId) {
        // 상태 업데이트 전에 유효성 검사
        console.log(
          `[${type}] 다음글 이동: ${currentId} -> ${result.nextId}, 카테고리: ${config.targetCategory}`
        );
